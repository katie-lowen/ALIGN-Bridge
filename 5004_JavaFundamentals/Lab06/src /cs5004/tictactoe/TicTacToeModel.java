...
/**
 * This creates a new TicTacToeModel object. 
 * @author Katie Lowen
 *
 */
public class TicTacToeModel implements TicTacToe {
  ...
  /**
   * Constructor of a new TicTacToeModel Object. 
   */
  ...
  public boolean isGameOver() {
    return (this.getWinner() != null || (Arrays.stream(board).flatMap(Arrays::stream)
        .filter(x -> x != null).count() == 9));
  ...
  //    for(int i = 0; i<2; i++) {
  //      for(int j = 0; j<2; j++) {
  //        if(this.board[i][j] != Player.O && this.board[i][j] != Player.X) {
  //          return false;
  //        }
  //      }
  //    }
