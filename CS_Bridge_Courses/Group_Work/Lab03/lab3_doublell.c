...
//Allocate memory to the heap
	...
	//is null is passed to the new lsit retrun zero.
	if ( newList == NULL) {
		return 0;}
	newList->head = NULL;
	newList-> tail = NULL;
...
//test to see if  avlaue is int he linked list or not 
bool boolean_in (node_t* node, int x) {
	if (node == NULL) {
	return false;}
	
	//goes throught he list unless value is found in which is breaks
...
int main() {
	...
	printf("this is queue tesing:");
	
	//intialize our lsit and our queue 
	slist_t* list = makeslist();
	//add three items to form queue 
	node_t*nothen = list->head;
	printf("first we will add a couple of nodes\n");
	add_end(list,10);
	printNodes(nothen);
	add_end(list,5);
	printNodes(nothen);
	add_end(list,8);
	printNodes(nothen);
	printf("have print statement to show queue forming \n");
	...
	remv(list,0);
	printNodes(node_5);
	printf("this is removing queue and queue goign empty \n");
	remv(list,0);
	remv(list,0);
	
	//adding up the list again
	add_end(list,43);
	add_end(list,23);
	printNodes(node_5);
	add_end(list,199);
	...
	printNodes(node_6);
	//removing a couple items to make sure it works still 
	remv(list,0);
	printNodes(node_6);
	remv(list,0);
	printNodes(node_6);
	
	//all print queue tests have the size and if the array is empty or not
	print_queue(list); 
	printf("second test \n \n");
		
	printf("this is for fun tesiing nothing to do with queues\n");
